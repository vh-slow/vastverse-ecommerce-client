...
import { ProductImageForm, ProductSpecForm, VariantForm } from '@/src/types';
...
export default function EditProductPage() {
    ...
    useEffect(() => {
        ...
        const loadProductDetail = async () => {
            try {
                ...
                try {
                    const catRes =
                        await apiCategory.getCategoriesDetailForAdmin(
                            data.categoryId
                        );
                    if (catRes) {
                        setSelectedCategory({
                            id: catRes.id,
                            name: catRes.name,
                            parentName: catRes.parentName || 'Danh mục',
                        });
                    } else {
                        setSelectedCategory({
                            id: data.categoryId,
                            name: `Danh mục #${data.categoryId}`,
                            parentName: '',
                        });
                    }
                } catch (e) {
                ...
                if (data.images && data.images.length > 0) {
                    const loadedImages: ProductImageForm[] = data.images.map(
                        (img: any) => ({
                            id: img.id,
                ...
                if (data.variants && data.variants.length > 0) {
                    setVariants(
                        data.variants.map((v: any) => ({
                            ...
                            initialStock: v.stockQuantity.toString(),
                            rawFile: null,
                            previewUrl: v.imageUrl,
                            originalImageUrl: v.imageUrl,
                        }))
                    );
                }
                ...
                try {
                    const specRes =
                        await apiProduct.getProductSpecifications(productId);
                    const specData = specRes?.data;

                    if (specData && specData.length > 0) {
                        setProductSpecs(
                            specData.map((s: any) => ({
                                tempId: `spec_${s.specificationId}_${Date.now()}`,
                    ...
                    console.error('Lỗi tải thông số kỹ thuật:', error);
                }
            } catch (error) {
                console.error(error);
                toast.error(t('products.update.messages.load_product_error'));
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (
                specsContainerRef.current &&
                !specsContainerRef.current.contains(event.target as Node)
            ) {
    ...
    useEffect(() => {
        const fetchCats = async () => {
            try {
                ...
                console.error(
                    t('products.update.messages.load_cat_error'),
                    error
                );
            }
        };
    ...
    const handleRemoveProductImage = (indexToRemove: number) => {
        setProductImages((prev) => {
            ...
            if (imgToRemove.id) {
                setDeletedImageIds((current) => [...current, imgToRemove.id!]);
            }
    ...
    const handleRemoveVariant = (tempId: string) => {
        if (variants.length === 1) {
            toast.error(t('products.update.messages.variant_limit'));
            return;
        }
    ...
    useEffect(() => {
        ...
        const fetchSpecs = async () => {
            try {
                ...
                console.error(
                    t('products.update.messages.load_spec_error'),
                    error
                );
            }
        };
        ...
        return () => clearTimeout(timer);
    }, [specSearch, activeSpecDropdown, t]);
    ...
    const handleSelectSpec = (tempId: string, spec: any) => {
        if (
            ...
            toast.error(t('products.update.messages.spec_duplicate'));
            return;
        }
    ...
    const handleCreateNewSpecSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success(t('products.update.messages.spec_created'));
            ...
            toast.error(
                error.response?.data?.message ||
                    t('products.update.messages.spec_create_error')
            );
        } finally {
    ...
    const handleSubmit = async (
        ...
        if (!productName.trim())
            return toast.error(t('products.update.messages.val_name_req'));
        if (!selectedCategory)
            return toast.error(t('products.update.messages.val_cat_req'));
        if (
            variants.some(
                (v) => !v.sku || !v.variantName || !v.price || !v.initialStock
            )
        ) {
            return toast.error(t('products.update.messages.val_var_req'));
        }
        if (productImages.length === 0)
            return toast.error(t('products.update.messages.val_img_req'));

        const invalidSpecs = productSpecs.filter(
            (s) => s.specificationId !== null && !s.value.trim()
        );
        if (invalidSpecs.length > 0)
            return toast.error(t('products.update.messages.val_spec_req'));
        ...
        try {
            ...
            const finalVariants = variants.map((v) => {
                ...
                return {
                    ...
                    stockQuantity: Number(v.initialStock),
            ...
            await apiProduct.updateProduct(productId, productPayload);

            const newGalleryImages = productImages.filter((p) => p.file);
            if (newGalleryImages.length > 0) {
                const formData = new FormData();
                newGalleryImages.forEach((p) =>
            ...
            toast.success(t('products.update.messages.update_success'));
            router.push('/admin/products');
        } catch (error: any) {
            console.error(error);
            toast.error(
                error.response?.data?.message ||
                    t('products.update.messages.update_error')
            );
        } finally {
    ...
    const breadcrumbs = [
        ...
        {
            name: t('products.title', { defaultValue: 'Sản phẩm' }),
            href: '/admin/products',
        },
        { name: t('products.update.title') },
    ];
    ...
    return (
        <>
            <Header
                title={t('products.update.title')}
                breadcrumbs={breadcrumbs}
            />

            <div className="flex-1 px-6 py-6 max-w-7xl mx-auto w-full relative">
                <form
                    className="space-y-6"
                ...
                >
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <h2 className="text-lg font-bold text-gray-800 mb-5 pb-3 border-b border-gray-100">
                            {t('products.update.basic_info.title')}
                        </h2>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    {t('products.update.basic_info.name_label')}{' '}
                                ...
                                <input
                                    ...
                                    placeholder={t(
                                        'products.update.basic_info.name_placeholder'
                                    )}
                                    required
                                />
                            </div>
                            ...
                            <div className="relative" ref={catDropdownRef}>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    {t(
                                        'products.update.basic_info.category_label'
                                    )}{' '}
                                    <span className="text-red-500">*</span>
                                </label>
                                <div
                                ...
                                >
                                    ...
                                    >
                                        {selectedCategory
                                            ? `${selectedCategory.name} (${t('products.update.basic_info.category_parent')}${selectedCategory.parentName})`
                                            : t(
                                                  'products.update.basic_info.category_placeholder'
                                              )}
                                    </span>
                                        ...
                                        <div className="p-3 border-b border-gray-100 bg-gray-50">
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    placeholder={t(
                                                        'products.update.basic_info.category_search'
                                                    )}
                                        ...
                                        <ul className="max-h-60 overflow-y-auto">
                                            {categories.length === 0 ? (
                                                <li className="px-4 py-3 text-sm text-gray-500 text-center">
                                                    {t(
                                                        'products.update.basic_info.category_not_found'
                                                    )}
                                                </li>
                                            ) : (
                                                categories.map((cat) => (
                                                    ...
                                                    >
                                                        ...
                                                        <span className="text-xs text-gray-500">
                                                            {t(
                                                                'products.update.basic_info.category_parent'
                                                            )}
                                                            {cat.parentName}
                                                        </span>
                                                    </li>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    {t(
                                        'products.update.basic_info.status_label'
                                    )}
                                </label>
                                <select
                                    value={status}
                                ...
                                >
                                    <option value={1}>
                                        {t(
                                            'products.update.basic_info.status_active'
                                        )}
                                    </option>
                                    <option value={0}>
                                        {t(
                                            'products.update.basic_info.status_draft'
                                        )}
                                    </option>
                                </select>
                            </div>
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    {t('products.update.basic_info.desc_label')}
                                </label>
                                <textarea
                                    rows={4}
                                    value={description}
                                    ...
                                    placeholder={t(
                                        'products.update.basic_info.desc_placeholder'
                                    )}
                                ></textarea>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 pb-4 border-b border-gray-100 gap-4">
                            <div>
                                <h2 className="text-lg font-bold text-gray-800">
                                    {t('products.update.gallery.title')}{' '}
                                    <span className="text-red-500">*</span>
                                </h2>
                                <p className="text-sm text-gray-500 mt-1">
                                    {t('products.update.gallery.subtitle')}
                                </p>
                            </div>
                            ...
                            <label className="shrink-0 px-5 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-600 font-semibold rounded-lg border border-blue-200 cursor-pointer transition-colors flex items-center gap-2 shadow-sm text-sm">
                                ...
                                {t('products.update.gallery.upload_btn')}
                                <input
                                    type="file"
                                    multiple
                                    accept="image/*"
                            ...
                            {Array.from({
                                ...
                                if (isUploaded) {
                                    ...
                                    return (
                                        <div
                                            key={idx}
                                            className={`aspect-square relative rounded-xl border-2 overflow-hidden group transition-all ${idx === 0 ? 'border-blue-500 shadow-md' : 'border-gray-200 hover:border-blue-300'}`}
                                        >
                                            <img
                                                src={getImageUrl(
                                                    imgObj.preview
                                            ...
                                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-2">
                                                <div className="flex justify-end">
                                                    <button
                                                        ...
                                                        title={t(
                                                            'products.update.gallery.delete_title'
                                                        )}
                                                    >
                                                ...
                                                {idx !== 0 && (
                                                    ...
                                                    >
                                                        {t(
                                                            'products.update.gallery.set_main_btn'
                                                        )}
                                                    </button>
                                                )}
                                            </div>
                                            ...
                                            {idx === 0 && (
                                                <div className="absolute bottom-0 left-0 right-0 bg-blue-600/95 text-white text-[10px] text-center py-1.5 font-bold uppercase tracking-wider">
                                                    {t(
                                                        'products.update.gallery.main_image_label'
                                                    )}
                                                </div>
                                            )}
                                        </div>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <div className="flex justify-between items-center mb-5 pb-3 border-b border-gray-100">
                            <div>
                                <h2 className="text-lg font-bold text-gray-800">
                                    {t('products.update.variants.title')}{' '}
                                    <span className="text-red-500">*</span>
                                </h2>
                                <p className="text-sm text-gray-500">
                                    {t('products.update.variants.subtitle')}
                                </p>
                            </div>
                            <button
                                type="button"
                            ...
                            >
                                ...
                                {t('products.update.variants.add_btn')}
                            </button>
                        </div>
                                            ...
                                            {variant.previewUrl ? (
                                                ...
                                                <>
                                                    ...
                                                    <span className="text-[10px] text-gray-500">
                                                        {t(
                                                            'products.update.variants.image_placeholder'
                                                        )}
                                                    </span>
                                                </>
                                            )}
                                    ...
                                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                        <div>
                                            <label className="block text-xs font-semibold text-gray-600 mb-1 uppercase">
                                                {t(
                                                    'products.update.variants.name_label'
                                                )}
                                            </label>
                                            <input
                                                type="text"
                                                placeholder={t(
                                                    'products.update.variants.name_placeholder'
                                                )}
                                        ...
                                        <div>
                                            <label className="block text-xs font-semibold text-gray-600 mb-1 uppercase">
                                                {t(
                                                    'products.update.variants.sku_label'
                                                )}
                                            </label>
                                            <input
                                                type="text"
                                                placeholder={t(
                                                    'products.update.variants.sku_placeholder'
                                                )}
                                                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm outline-none focus:border-blue-500"
                                                value={variant.sku}
                                                onChange={(e) =>
                                        ...
                                        <div>
                                            <label className="block text-xs font-semibold text-gray-600 mb-1 uppercase">
                                                {t(
                                                    'products.update.variants.price_label'
                                                )}
                                            </label>
                                            <input
                                                type="number"
                                                min="0"
                                                placeholder={t(
                                                    'products.update.variants.price_placeholder'
                                                )}
                                                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm outline-none focus:border-blue-500"
                                                value={variant.price}
                                                onChange={(e) =>
                                        ...
                                        <div>
                                            <label className="block text-xs font-semibold text-gray-600 mb-1 uppercase">
                                                {t(
                                                    'products.update.variants.stock_label'
                                                )}
                                            </label>
                                            <input
                                                type="number"
                                                min="0"
                                                placeholder={t(
                                                    'products.update.variants.stock_placeholder'
                                                )}
                                                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm outline-none focus:border-blue-500"
                    ...
                    >
                        ...
                        >
                            <div>
                                <h2 className="text-lg font-bold text-gray-800">
                                    {t('products.update.specs.title')}
                                </h2>
                                <p className="text-sm text-gray-500 font-normal mt-1">
                                    {t('products.update.specs.subtitle')}
                                </p>
                            </div>
                                ...
                                {productSpecs.length === 0 ? (
                                    <div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                        ...
                                        <p className="text-gray-500 text-sm mb-4">
                                            {t(
                                                'products.update.specs.empty_state'
                                            )}
                                        </p>
                                        <button
                                            type="button"
                                        ...
                                        >
                                            ...
                                            {t(
                                                'products.update.specs.add_first_btn'
                                            )}
                                        </button>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        <div className="hidden md:grid grid-cols-12 gap-4 px-4 py-2 bg-gray-100 rounded-lg text-xs font-bold text-gray-600 uppercase tracking-wide">
                                            <div className="col-span-4">
                                                {t(
                                                    'products.update.specs.col_name'
                                                )}
                                            </div>
                                            <div className="col-span-7">
                                                {t(
                                                    'products.update.specs.col_value'
                                                )}
                                            </div>
                                            <div className="col-span-1 text-center">
                                                {t(
                                                    'products.update.specs.col_action'
                                                )}
                                            </div>
                                        </div>
                                            ...
                                            >
                                                <div className="w-full md:col-span-4 relative">
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            {spec.specificationName ||
                                                                t(
                                                                    'products.update.specs.name_placeholder'
                                                                )}
                                                        </span>
                                                        ...
                                                        <div className="absolute z-40 top-full left-0 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden">
                                                            <div className="p-2 border-b border-gray-100 bg-gray-50">
                                                                <input
                                                                    ...
                                                                    placeholder={t(
                                                                        'products.update.specs.search_placeholder'
                                                                    )}
                                                                    value={
                                                                        specSearch
                                                                    }
                                                            ...
                                                            <ul className="max-h-48 overflow-y-auto">
                                                                ...
                                                                0 ? (
                                                                    <li className="px-3 py-4 text-xs text-gray-500 text-center">
                                                                        {t(
                                                                            'products.update.specs.not_found'
                                                                        )}
                                                                    </li>
                                                                ) : (
                                                                    fetchedSpecs.map(
                                                            ...
                                                            <div className="p-2 bg-gray-50 border-t border-gray-200 text-center">
                                                                ...
                                                                >
                                                                    ...
                                                                    {t(
                                                                        'products.update.specs.create_new_btn'
                                                                    )}
                                                                </button>
                                                            </div>
                                                        </div>
                                                ...
                                                <div className="w-full md:col-span-7">
                                                    ...
                                                    <input
                                                        type="text"
                                                        placeholder={t(
                                                            'products.update.specs.value_placeholder'
                                                        )}
                                                        className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm outline-none focus:border-blue-500"
                                                        value={spec.value}
                                                        onChange={(e) =>
                                        ...
                                        <div className="pt-2">
                                            ...
                                            >
                                                ...
                                                {t(
                                                    'products.update.specs.add_row_btn'
                                                )}
                                            </button>
                                        </div>
                                    </div>
                    ...
                    <div className="flex justify-end gap-3 pb-10">
                        ...
                        >
                            {t('products.update.actions.cancel')}
                        </button>
                        <button
                            type="button"
                            ...
                            className="px-6 py-2.5 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition-colors disabled:opacity-70"
                        >
                            {t('products.update.actions.save_draft')}
                        </button>
                        <button
                            type="submit"
                            disabled={isSubmitting}
                        ...
                        >
                            {isSubmitting ? (
                                <>
                                    ...
                                    {t('products.update.actions.saving')}
                                </>
                            ) : (
                                <>
                                    <i className="fa-solid fa-check"></i>{' '}
                                    {t('products.update.actions.save')}
                                </>
                            )}
                        </button>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-up">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                            <h3 className="text-lg font-bold text-gray-800">
                                {t('products.update.spec_modal.title')}
                            </h3>
                            <button
                                type="button"
                        ...
                        <form onSubmit={handleCreateNewSpecSubmit}>
                            <div className="p-6">
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    {t('products.update.spec_modal.name_label')}{' '}
                                    <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    ...
                                    placeholder={t(
                                        'products.update.spec_modal.name_placeholder'
                                    )}
                                ...
                                <p className="text-xs text-gray-500 mt-2">
                                    ...
                                    {t('products.update.spec_modal.hint')}
                                </p>
                            </div>
                            <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                                ...
                                >
                                    {t('products.update.spec_modal.cancel_btn')}
                                </button>
                                <button
                                    type="submit"
                                ...
                                >
                                    {isCreatingSpec ? (
                                        ...
                                        t(
                                            'products.update.spec_modal.submit_btn'
                                        )
                                    )}
                                </button>
                            </div>

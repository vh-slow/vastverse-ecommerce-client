...
export default function UpdateSupplierPage() {
    ...
    useEffect(() => {
        ...
        const loadSupplierDetail = async () => {
            try {
                const data = await apiSupplier.getSupplierById(supplierId);
                ...
                setStatus(data.status !== undefined ? data.status : 1);
                ...
                console.error(error);
                toast.error(t('suppliers.update.messages.load_error'));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!name.trim() || !contactName.trim()) {
            toast.error(t('suppliers.update.messages.val_info_req'));
            return;
        }
        ...
        try {
            ...
            await apiSupplier.updateSupplier(supplierId, formData);

            toast.success(t('suppliers.update.messages.update_success'));
            router.push('/admin/suppliers');
        } catch (error: any) {
            console.error('Update supplier error:', error);
            toast.error(
                error.response?.data?.message ||
                    t('suppliers.update.messages.update_error')
            );
        } finally {
        ...
        <>
            ...
            <div className="flex-1 px-6 py-2">
                <form className="space-y-8" onSubmit={handleSubmit}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder={t(
                                        'suppliers.update.form.name_placeholder'
                                    )}
                                    required
                                    autoFocus
                                />
                            ...
                            <div>
                                <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                    {t('suppliers.update.form.contact_label')}{' '}
                                    <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    ...
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition"
                                    placeholder={t(
                                        'suppliers.update.form.contact_placeholder'
                                    )}
                                    required
                                />
                            </div>
                            ...
                            <div>
                                <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                    {t('suppliers.update.form.phone_label')}
                                </label>
                                <input
                                    type="text"
                                    value={phone}
                                    ...
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition"
                                    placeholder={t(
                                        'suppliers.update.form.phone_placeholder'
                                    )}
                                />
                            </div>
                            ...
                            <div>
                                <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                    {t('suppliers.update.form.email_label')}
                                </label>
                                <input
                                    type="email"
                                    value={email}
                                    ...
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition"
                                    placeholder={t(
                                        'suppliers.update.form.email_placeholder'
                                    )}
                                />
                            </div>
                            ...
                            <div>
                                <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                    {t('suppliers.update.form.tax_label')}
                                </label>
                                <input
                                    type="text"
                                    value={taxCode}
                                    ...
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition"
                                    placeholder={t(
                                        'suppliers.update.form.tax_placeholder'
                                    )}
                                />
                            </div>
                            ...
                            <div>
                                <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                    {t('suppliers.update.form.status_label')}
                                </label>
                                <select
                                    value={status}
                                    onChange={(e) =>
                            ...
                            <div className="md:col-span-2">
                                <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                    {t('suppliers.update.form.address_label')}
                                </label>
                                <input
                                    type="text"
                                    value={address}
                                    ...
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition"
                                    placeholder={t(
                                        'suppliers.update.form.address_placeholder'
                                    )}
                                />
                            </div>
                        </div>
                    ...
                    <div className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm">
                        <h2 className="text-xl font-semibold text-gray-800 mb-6 pb-4 border-b border-gray-200">
                            {t('suppliers.update.card_media')}
                        </h2>

                        <div>
                            <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                {t('suppliers.update.form.logo_label')}
                            </label>

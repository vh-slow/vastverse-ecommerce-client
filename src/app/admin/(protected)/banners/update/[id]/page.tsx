...
export default function UpdateBannerPage() {
    ...
    const [originalPlacement, setOriginalPlacement] = useState<number>(1);
    ...
    useEffect(() => {
        ...
        const loadBannerDetail = async () => {
            try {
                const data = await apiBanner.getBannerById(bannerId);

                setPlacement(data.placementId);
                setOriginalPlacement(data.placementId);
                ...
                if (data.linkUrl && data.linkUrl.startsWith('/products/')) {
                    const slug = data.linkUrl.replace('/products/', '');
                    setSelectedProduct({
                        slug: slug,
                        name: `Sản phẩm: ${slug}`,
                    });
                ...
                if (data.placementId === 1 && data.imageUrl) {
                    setMediaPreview(getImageUrl(data.imageUrl));
                } else if (data.placementId === 2 && data.videoUrl) {
                    setMediaPreview(getImageUrl(data.videoUrl));
                }
            } catch (error) {
                console.error(error);
                toast.error(t('banners.update.messages.load_error'));
    ...
    useEffect(() => {
        if (!isLoading && placement !== originalPlacement) {
            setMediaFile(null);
            setMediaPreview(null);
        }
    }, [placement, originalPlacement, isLoading]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!title.trim() || !subtitle.trim() || !buttonText.trim()) {
            toast.error(t('banners.update.messages.val_info_req'));
            return;
        }

        if (!selectedProduct) {
            toast.error(t('banners.update.messages.val_link_req'));
            return;
        }

        if (!mediaPreview && !mediaFile) {
            toast.error(t('banners.update.messages.val_media_req'));
            return;
        }
        ...
        try {
            ...
            await apiBanner.updateBanner(bannerId, formData);

            toast.success(t('banners.update.messages.update_success'));
            router.push('/admin/banners');
        } catch (error: any) {
            console.error('Update banner error:', error);
            toast.error(
                error.response?.data?.message ||
                    t('banners.update.messages.update_error')
            );
        } finally {
        ...
        <>
            ...
            <div className="flex-1 px-6 py-2">
                <form className="space-y-8" onSubmit={handleSubmit}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                >
                                    <option value={1}>
                                        {t(
                                            'banners.update.form.placement_home'
                                        )}
                                    </option>
                                    <option value={2}>
                                        {t(
                                            'banners.update.form.placement_portfolio'
                                        )}
                                    </option>
                                </select>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                        {t('banners.update.form.status_label')}
                                    </label>
                                    <select
                                        value={isActive}
                                        onChange={(e) =>
                                ...
                                <div>
                                    <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                        {t('banners.update.form.order_label')}{' '}
                                    ...
                                    <input
                                        ...
                                        placeholder={t(
                                            'banners.update.form.order_placeholder'
                                        )}
                                        required
                                    />
                                </div>
                            ...
                            <div className="md:col-span-2">
                                <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                    {t('banners.update.form.title_label')}{' '}
                                    <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    ...
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition"
                                    placeholder={t(
                                        'banners.update.form.title_placeholder'
                                    )}
                                    required
                                />
                            </div>
                            ...
                            <div>
                                <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                    {t('banners.update.form.subtitle_label')}{' '}
                                    <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    ...
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition"
                                    placeholder={t(
                                        'banners.update.form.subtitle_placeholder'
                                    )}
                                    required
                                />
                            </div>
                            ...
                            <div>
                                <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                    {t('banners.update.form.button_label')}{' '}
                                    <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    ...
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition"
                                    placeholder={t(
                                        'banners.update.form.button_placeholder'
                                    )}
                                    required
                                />
                            </div>
                            ...
                            <div className="md:col-span-2">
                                <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                    {t('banners.update.form.desc_label')}
                                </label>
                                <textarea
                                    rows={3}
                                    value={description}
                                    ...
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition"
                                    placeholder={t(
                                        'banners.update.form.desc_placeholder'
                                    )}
                                ></textarea>
                            </div>
                        </div>
                    ...
                    <div className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm">
                        <h2 className="text-xl font-semibold text-gray-800 mb-6 pb-4 border-b border-gray-200">
                            {t('banners.update.card_link')}
                        </h2>

                        <div className="relative" ref={productDropdownRef}>
                            <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                {t('banners.update.form.product_label')}{' '}
                                <span className="text-red-500">*</span>
                            </label>
                            <div
                            ...
                            >
                                ...
                                >
                                    {selectedProduct
                                        ...
                                        : t(
                                              'banners.update.form.product_placeholder'
                                          )}
                                </span>
                                    ...
                                    <div className="p-3 border-b border-gray-100 bg-gray-50">
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                placeholder={t(
                                                    'banners.update.form.product_search'
                                                )}
                                                value={productSearch}
                                                onChange={(e) =>
                                                    setProductSearch(
                                    ...
                                    <ul className="max-h-72 overflow-y-auto">
                                        {products.length === 0 ? (
                                            <li className="px-4 py-4 text-sm text-gray-500 text-center">
                                                {t(
                                                    'banners.update.form.product_not_found'
                                                )}
                                            </li>
                                        ) : (
                                            products.map((prod) => (
                    ...
                    <div className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm">
                        <h2 className="text-xl font-semibold text-gray-800 mb-6 pb-4 border-b border-gray-200">
                            {t('banners.update.card_media')}
                        </h2>

                        <div>
                            <label className="form-label mb-2 block text-sm font-medium text-gray-700">
                                {placement === 1
                                    ? t('banners.update.media.image_label')
                                    : t(
                                          'banners.update.media.video_label'
                                      )}{' '}
                                <span className="text-red-500">*</span>
                            </label>
                                ...
                                {mediaPreview ? (
                                    ...
                                    <div className="text-center z-10 pointer-events-none">
                                        ...
                                        <p className="text-xs text-gray-500 mt-1">
                                            {placement === 1
                                                ? t(
                                                      'banners.update.media.image_hint'
                                                  )
                                                : t(
                                                      'banners.update.media.video_hint'
                                                  )}
                                        </p>
                                    </div>
                                )}

...
export default function UpdateWarehousePage() {
    ...
    useEffect(() => {
        ...
        const loadWarehouseDetail = async () => {
            setIsFetching(true);
            try {
                const data = await apiWarehouse.getWarehouseById(
                    Number(warehouseId)
                );
                setName(data.name);
                ...
                setProvinceId(data.provinceId === 0 ? '' : data.provinceId);
                setDistrictId(data.districtId === 0 ? '' : data.districtId);
                setWardCode(data.wardCode);
                setIsActive(data.isActive);
            } catch (error) {
                console.error(error);
                toast.error(t('warehouses.update.messages.fetch_error'));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await apiWarehouse.updateWarehouse(Number(warehouseId), payload);

            toast.success(t('warehouses.update.messages.update_success'));
            router.push('/admin/warehouses');
        } catch (error: any) {
            console.error('Update warehouse error:', error);
            toast.error(
                error.response?.data?.message ||
                    t('warehouses.update.messages.update_error')
            );
        } finally {
    ...
    const breadcrumbs = [
        ...
        { name: t('warehouses.update.title') },
    ];
    ...
    return (
        <>
            <Header
                title={t('warehouses.update.title')}
                ...
                <form className="space-y-8" onSubmit={handleSubmit}>
                    <div className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm">
                        <h2 className="text-base font-semibold text-gray-800 mb-6 pb-4 border-b border-gray-200">
                            {t('warehouses.update.card_title')}
                        </h2>
                        ...
                        >
                            {isSubmitting ? (
                                <>
                                    ...
                                    {t('warehouses.update.actions.saving')}
                                </>
                            ) : (
                                <>
